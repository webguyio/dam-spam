<?php
// phpcs:disable WordPress.Security.NonceVerification.Missing -- Public registration form, repopulating form values after error
if ( !defined( 'ABSPATH' ) ) {
	status_header( 404 );
	exit;
}
...
<form name="registerform" id="registerform" action="<?php echo esc_url( home_url( '/register/' ) ); ?>" method="post" novalidate="novalidate">
	...
	<p class="dam-spam-input-wrapper url">
	...
	<?php do_action( 'register_form' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WordPress core hook ?>
...
<style>
p.dam-spam-input-wrapper label, #registerform label {
	display: block;
}
p.dam-spam-input-wrapper .input, #registerform .input {
	padding: 15px;
	min-width: 50%;
}
p.dam-spam-input-wrapper.url {
